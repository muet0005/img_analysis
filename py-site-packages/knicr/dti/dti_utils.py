...
class knicrDTIprep():
    ...
    def fit_restore(self):
        ...
        restore_tensor = os.path.join(oDir, 'tensor.restore.Bdouble')
        residual = os.path.join(oDir, 'residual.restore.Bdouble')
        if not os.path.exists(restore_tensor):
            ...
            modelFit = camino.ModelFit(in_file=dwi_camino, model='restore', scheme_file=bvec_camino, bgthresh=1, residualmap=residual,sigma=sigma, out_file=restore_tensor, ignore_exception=True)
            modelFit.run()
        ...
        opts = ['voxel2image', '-outputroot', 'restore', residual.replace('.Bdouble', '.nii'), '-gzip']
        p = sp.Popen(opts)
        p.wait()
        write_log(str(opts), logFile)
        #cleanup
