...
class knicrTBSS():
        ...
        for i in kwargs.keys():
            if i == 'fa':
                ...
                #this causes major problems with the erosion.  Further, the susceptibility artifact also causes masking problems for tbss.
                    ...
                    fa = os.path.join(dtiDir, subject, 'dmri', 'fsl', 'dtifit_FA.nii.gz')
    ...
    def tbss_3_postregB(self, subjList, merged4d, **kwargs):
        ...
        fslmaths.run()
        write_log(fslmaths.cmdline, logFile)
        opt_str = '-mas ' + os.path.join(self.statsDir, 'mean_FA_mask.nii.gz')
    ...
    def tbss_4_prestats(self, **kwargs):
        ...
        opt_str = '-mul 1 -add 1 -add ' + os.path.join(self.statsDir, 'mean_FA_skeleton_mask.nii.gz')
        ...
        if x == 1 and y == 1 and z == 1:
            ...
            if not os.path.exists(cing_file):
                ...
                flirt = fsl.FLIRT(in_file=cing_file1mm, reference=os.path.join(os.environ['FSLDIR'], 'data', 'standard', 'MNI152_2mm_T1.nii.gz'), out_file=cing_file, args='-applyisoxfm 2')
                flirt.run()
                write_log(flirt.cmdline, logFile)
        print 'Projecting all FA data onto Skeleton'
        final4d = self.merged4d.replace('.nii.gz', '_skeletonized.nii.gz')
        tbss_skeleton = fsl.TractSkeleton(in_file=os.path.join(self.statsDir, 'mean_FA.nii.gz'), threshold=threshold, 
        distance_map=os.path.join(self.statsDir, 'mean_FA_skeleton_mask_dst.nii.gz'), search_mask_rule=cing_file, data_file=self.merged4d, projected_data=final4d)
        tbss_skeleton.run()
        write_log(tbss_skeleton.cmdline, logFile)
