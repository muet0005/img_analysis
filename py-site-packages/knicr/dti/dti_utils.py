...
class knicrDTIprep():
    ...
    def mergeDwis(self, **kwargs):
        ...
        def mergeBtables():
            ...
            oBvecFile.close()
            oBvalFile.close()
        def transposeBtable(bTable, oBtable):
            b = open(bTable, 'r')
            ...
            for i in csvB:
                ...
                while removeWs:
                    try:
                        ...
                        removeWs = False
                print bFlipInfo['x'], bFlipInfo['y'], bFlipInfo['z']
                print i
                x=float(i[0])*bFlipInfo['x']
                y=float(i[1])*bFlipInfo['y']
                z=float(i[2])*bFlipInfo['z']
                if not bData.has_key('x'):
                    bData['x'] = [x]
                else:
                    bData['x'].append(x)	
                if not bData.has_key('y'):
                    bData['y'] = [y]
                else:
                    bData['y'].append(y)		
                if not bData.has_key('z'):
                    bData['z'] = [z]
                else:
                    bData['z'].append(z)
            o = open(oBtable, 'w')
            bWrite = csv.writer(o, delimiter=' ')
            bWrite.writerow(bData['x'])
            bWrite.writerow(bData['y'])
            bWrite.writerow(bData['z'])
            o.close()
            b.close()
        if bTable == 'merge':
            mergeBtables()
