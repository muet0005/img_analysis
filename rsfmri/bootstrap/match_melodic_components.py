...
def compute_ui(trg_map, src_map):
	...
	trg_nii = nb.load(trg_map)
	src_nii = nb.load(src_map)
	#pull out the data array
	trg_data = trg_nii.get_data()
	src_data = src_nii.get_data()
...
matched_components = compute_ui(trg_map, src_map, matched_components)
