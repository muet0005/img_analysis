	...
	def gen_melodic_lisa(self, fmri_dir, feat_sfix, melodic_sample_dir, samples_sfix):
		...
		for s in range(0, nSamples):
			...
			if not os.path.exists(odir):
				os.makedirs(odir)
			oFile = os.path.join(melodic_sample_dir, 'sample.' + str(s) + '.' + samples_sfix, 'sample.' + str())
			if not os.path.exists(oFile):
				f = open(oFile, 'w')
				for l in self.samples[s]:
					f.write(os.path.join(fmri_dir, str(l),  str(l) + feat_sfix, 'filtered_func_data.nii.gz' + '\n'))
				f.close()
			else:
