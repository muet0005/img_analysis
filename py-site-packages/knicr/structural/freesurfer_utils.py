...
class surfaceAnalysis():
    ...
    def mriGlmFitSim(self, **kwargs):
        ...
        if simType == 'mc-z':
            ...
            opts= ['mri_glmfit-sim', '--glmdir', self.glmdir, '--cache', thr, simSign, '--cwp', cwp]
            if bothHemis:
                opts.append('--2spaces')
        else:
...
class extractFreeSurferStats():
    def __init__(self, subjects_dir, **kwargs):
        ...
        con = mysqldb.connect(db = self.mydb, read_default_file=os.path.join(os.getenv("HOME"), '.my.cnf'))
    ...
    def dumpAparcStats(self, subject):
    	...
    	try:
    		self.cursor.execute("""create table %s (%s varchar(10))""" % (tbl, self.id_col))
    	except:
    		pass
    	#check the table to see if the subject exists
    ...
    def dumpWmparcStats(self, subject):
        ...
        def dumpTbvStats(subjects_dir, idc):
        	stats_csv = ''
        	stats_file = ''
        	tbl = tblroot + '_tbv_stats'
        	id_col = 'IDC'
        	idval = idc
        	try:
        		cursor.execute("""create table %s (%s varchar(10))""" % (tbl, id_col))
        	except:
        		pass
            #check the table to see if the subject exists
        	cursor.execute("""select %s from %s where %s=\'%s\'""" % (id_col, tbl, id_col, idval))
        	pn_exist = cursor.fetchone()
        	if not pn_exist:
        		cursor.execute("""insert into %s (%s) values (\'%s\')""" % (tbl, id_col, idval))
            #specify the file to read in
        	stats_dir =os.path.join(subjects_dir, idc, 'stats')
        	stats_file = open(os.path.join(stats_dir, 'aseg.stats'), 'r')
        	stats_csv = csv.reader(stats_file, delimiter=',')	
        	for row in stats_csv:
        		if len(row) == 5:		
        			if row[1].strip() == 'lhCortexVol':
        				...
        				col_vol = roi
        				try:
        					cursor.execute("""alter table %s add column (%s float)""" % (tbl, col_vol))
        				except:
        					pass
        				cursor.execute("""update %s set %s=%s where %s=\'%s\'""" % (tbl, col_vol, vol, id_col, idval))
        			if row[1].strip() == 'rhCortexVol':
        				roi = row[1].strip()	
        				vol = float(row[3])
        				col_vol = roi
        				try:
        			...
        			if row[1].strip() == 'CortexVol':
        				...
        				try:
        					...
        					pass
        				cursor.execute("""update %s set %s=%s where %s=\'%s\'""" % (tbl, col_vol, vol, id_col, idval))
        			if row[1].strip() == 'lhCorticalWhiteMatterVol':
        				roi = row[1].strip()	
        				vol = float(row[3])
        				col_vol = roi
        				try:
        					...
        					pass
        				cursor.execute("""update %s set %s=%s where %s=\'%s\'""" % (tbl, col_vol, vol, id_col, idval))
        			if row[1].strip() == 'rhCorticalWhiteMatterVol':
        				roi = row[1].strip()	
        				vol = float(row[3])
        				col_vol = roi
        				try:
        					...
        					pass
        				cursor.execute("""update %s set %s=%s where %s=\'%s\'""" % (tbl, col_vol, vol, id_col, idval))
        			if row[1].strip() == 'CorticalWhiteMatterVol':
        				roi = row[1].strip()	
        				vol = float(row[3])
        				col_vol = roi
        				try:
        			...
        			if row[1].strip() == 'TotalGrayVol':
        				...
        				try:
        					...
        					pass
        				cursor.execute("""update %s set %s=%s where %s=\'%s\'""" % (tbl, col_vol, vol, id_col, idval))
        			if row[1].strip() == 'SupraTentorialVol':
        				roi = row[1].strip()	
        				vol = float(row[3])
        				col_vol = roi
        				try:
        					...
        					pass
        				cursor.execute("""update %s set %s=%s where %s=\'%s\'""" % (tbl, col_vol, vol, id_col, idval))
        			if row[1].strip() == 'SupraTentorialVolNotVent':
        				roi = row[1].strip()	
        				vol = float(row[3])
        				col_vol = roi
        				try:
        					...
        					pass
        				cursor.execute("""update %s set %s=%s where %s=\'%s\'""" % (tbl, col_vol, vol, id_col, idval))
        			if row[1].strip() == 'SupraTentorialVolNotVentVox':
        				roi = row[1].strip()	
        				vol = float(row[3])
        				col_vol = roi
        				try:
    	...
    	for row in stats_csv:
    		if len(row) == 5:
    			...
    			if row[1].strip() == 'lhCortexVol':
    				...
    				vol = float(row[3])
    				col_vol = roi + self.varSfix
    			elif row[1].strip() == 'lhCorticalWhiteMatterVol':
    				roi = row[1].strip()	
    				vol = float(row[3])
    				col_vol = roi + self.varSfix
    			elif row[1].strip() == 'rhCorticalWhiteMatterVol':
    				roi = row[1].strip()	
    				vol = float(row[3])
    				col_vol = roi + self.varSfix
    			elif row[1].strip() == 'CorticalWhiteMatterVol':
    				roi = row[1].strip()	
    				vol = float(row[3])
    				col_vol = roi + self.varSfix
