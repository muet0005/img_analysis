...
class MRILabel2LabelInputSpec(FSTraitedSpec):
    """
    Uses Freesurfer's mri_label2label to map labels to different spaces
    """
    srclabel = File(exists=True, argstr="--srclabel %s",
    desc="input label file")
    srcsubject = File(exists=False, argstr="--srcsubject %s",
    desc="source subject")
    trgsubject = File(exists=False, argstr="--trgsubject %s",
    desc="target subject")
    ...
    hemi = File(exists=False, argstr="--hemi %s",
    desc="hemi to use")
    



class MRILabel2LabelOutputSpec(TraitedSpec):
    """
    Uses Freesurfer's mri_label2label to map labels to different spaces
    """
    label = File(exists=True, desc='label output file')
