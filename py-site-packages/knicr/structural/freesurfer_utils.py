...
class qdec_clusters():
    ...
    def computeQdecLabelStats(self, label, hemi, **kwargs):
        ...
        for s in subjects:
            ...
            print labelfile
            if not os.path.exists(labelfile) or s.startswith(exclude):
    ...
    def dumpQdecLabelStatsMysql(self, cursor, label, hemi, **kwargs):
        ...
        cursor.execute("""select %s from %s where %s=%s""" % (id_col, tbl, id_col, idc))
        pn_exist = cursor.fetchone()
        if not pn_exist:
            cursor.execute("""insert into %s (%s) values (%s)""" % (tbl, id_col, idc))
        #specify the file to read in
        stats_file = open(os.path.join(subjects_dir, idc, 'stats', os.path.basename(label) + '.' + measure + '.stats'), 'r')
            ...
            if row[0] != '#':
                for i in range(0, len(row)):
                    ...
                    try:
                        ...
                        pass
                    cursor.execute("""update %s set %s=%s where %s=%s""" % (tbl, col, val, id_col, idc))
                    if val_std:
                        try:
                            cursor.execute("""alter table %s add column (%s float)""" % (tbl, col_std))
                        except:
                            pass
                        cursor.execute("""update %s set %s=%s where %s=%s""" % (tbl, col_std, val_std, id_col, idc))
