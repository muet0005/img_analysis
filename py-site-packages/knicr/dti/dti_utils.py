...
class knicrTBSS():
    ...
    def tbss_1_preproc(self, dtiDir, subject, **kwargs):
        ...
        self.mask = new_fa.replace('.nii.gz', '_mask.nii.gz')
    ...
    def tbss_2_reg(self, subject):
        ...
        oFile = obn + '.nii.gz'
        mat = obn + '.mat'
        logFile = os.path.join(self.faDir, 'idc_' + subject + '.tbss_2_reg.log')
        if not os.path.exists(oFile):
            flirt = fsl.FLIRT(in_file=self.fa, reference=self.template, out_file=oFile, out_matrix_file=mat)
            flirt.run()
