...
class qdec_clusters():
    ...
    def computeQdecLabelStats(self, label, hemi, **kwargs):
        ...
        Optional Arguments:
        Measure: This is the kind of stats to extract. Options are thickness, volume (gray) lgi, and area (gray).
        ...
        for s in subjects:
            ...
            if measure == 'thickness' or measure == 'volume':
    ...
    def dumpQdecLabelStatsMysql(self, cursor, label, hemi, **kwargs):
        ...
        Optional:
            ...
            measure -> the type of measure to extract. thicknes is default. other opts are lgi, volume (gray), and area (gray matter)
                    other measures: irmcurv (Integrated Rectified Mean Curvature, 1/mm), irgcurv (Integrated Rectified Gaussian Curvature, 1/mm2), 
                    foldind (folding index van essen), intcurv (intrinsic curv, van essen) 
        ...
        for s in subjects:
            ...
            if measure == 'thickness' or measure == 'lgi' or measure == 'area' or measure == 'volume':
                stats_file = os.path.join(self.subjects_dir, s, 'stats', os.path.basename(label) + '.' + measure + '.stats')
            else:
                print 'Warning....curvature or folding selected....assuming area/pial stats file...'
                stats_file = os.path.join(self.subjects_dir, s, 'stats', os.path.basename(label) + '.area.stats')
                ...
                if row[0] != '#':
                    for i in range(0, len(row)):
                        ...
                        if measure == 'thickness':
                            ...
                            val = row[3]
                            col = roi.replace('.', '_') + '_gvolume'
                        elif measure == 'irmcurv':
                            val = row[6]
                            col = roi.replace('.', '_') + '_irmcurv'
                        elif measure == 'irgcurv':
                            val = row[7]
                            col = roi.replace('.', '_') + '_irgcurv'   
                        elif measure == 'foldind':
                            val = row[8]
                            col = roi.replace('.', '_') + '_foldind'
                        elif measure == 'intcurv':
                            val = row[9]
                            col = roi.replace('.', '_') + '_intcurv'
                        if val == '':
                            continue
                        val = float(val)
                        try:
