...
class surfaceAnalysis():
    def __init__(self, **kwargs):
        ...
        self.subjectsDir = False ; self.oDir = False ; self.outBaseName = False ; self.overwrite = False
        for i in kwargs.keys():
            if i == 'hemi':
                self.hemi = kwargs[i]
            elif i == 'fwhm':
                ...
                self.overwrite = True
            if not subjectsDir or not oDir or not outBaseName or not fsgdFile:
        ...
        for i in kwargs.keys():
            if i == 'glmdir':
                ...
                if not isinstance(contrasts, list):
                    ...
                    sys.exit(0)        
        y = os.path.join(self.oDir, self.hemi + '.' + self.outBaseName + '.' + self.measure + '.' + str(self.fwhm) + '.mgh')
        glm = freesurfer.GLMFit(in_file = y, contrast = self.contrasts, design = self.fsgdMat, surf = True, subject_id = self.template, hemi = self.hemi, cortex = True, glm_dir = glmdir)
        glm.run()
