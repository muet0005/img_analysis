...
def compute_stat(iData, mask, **kwargs):
    ...
    rescale = bool. True means the mask will be rescaled to be 0->1. This probably should be done on probtrack normalized distributions.?
    """
    stat = 'wavg'
    rescale = False
    ...
    if stat == 'wavg':
        ...
        val = np.ma.mean(np.ma.masked_where(mask_data<1, iData))
    elif stat == 'median':
        val = np.ma.median(np.ma.masked_where(mask_data<1, iData))
    elif stat == 'stdev':
        val = np.ma.std(np.ma.masked_where(mask_data<1, iData))
    return val
    ...
    def extract_fs_stats(self, tblroot, subjid, **kwargs):
        ...
        *Assumes knicrDTIreg has been run, and the labels have been mapped to native space.
        ...
        Optional inputs:    parcType    ---> default is wmparc
                                        ---> string format
                                        ---> Othersupported options: aseg, aparc
        ...
        tbl = tblroot + '_' + parcType + 'dti_stats'
        id_col = 'ID'
        idval = subjid
        try:
        ...
        labelDir = os.path.join(subjDir, 'label', 'wmparc', 'diff')
        ...
        for scalar in scalars:
            ...
            for label in labels:
                ...
                for stat in stats:
                    ...
                    val = compute_Waverage(data, os.path.join(labelDir, label, stat=stat))
                    if not val:
