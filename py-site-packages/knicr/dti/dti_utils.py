            ...
            if not os.path.exists(tracts):
                ...
                for m in mask_list:
                    if m == 'seed':
                        ...
                        istop_mni = os.path.join(masks, 'stop.nii.gz')
                        stop = Undefined
                ...
                if os.path.exists(os.path.join(masks, 'invert')):
                    invert = True
                fsamples = os.path.join(self.bpxDir, 'merged_f1samples.nii.gz')
                phsamples = os.path.join(self.bpxDir, 'merged_ph1samples.nii.gz')
                thsamples = os.path.join(self.bpxDir, 'merged_th1samples.nii.gz')
                ptx = fsl.ProbTrackX(seed=seed, waypoints=target, fsamples=fsamples, phsamples=phsamples, thsamples=thsamples, samples_base_name=os.path.join(self.bpxDir, 'merged'), 
                mask=os.path.join(self.subjDir, 'dmri', 'nodif_brain_mask.nii.gz'), stop_mask=stop, n_samples=nSeed, opd=True, 
                out_dir=os.path.join(tracts, 'tracts'), avoid_mp=avoid, loop_check=True, force_dir=True, terminal_output='stream')
                ptx.run()
...
class knicrTBSS():
    ...
    def tbss_nonFA_reg(self, subject, nonFA):
        ...
        iFile = os.path.join(self.dtiDir, subject, 'dmri', 'fsl', nonFA + '.nii.gz')
        if not os.path.exists(oFile):
