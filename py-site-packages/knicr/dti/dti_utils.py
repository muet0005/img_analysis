...
class knicrDTIFIT():
    ...
    def FSLditfit(subject, **kwargs):
        ...
        if not os.path.exists(os.path.dirname(obn)):
            os.makedirs(os.dirname(obn))
        if not os.path.exists(mask):
            bet = fsl.BET(frac=0.25, in_file=iFile, mask=True, out_file=mask)
            bet.run()
            logFile = os.path.join(os.path.dirname(mask), 'bet.log')
            write_log(bet.cmdline, logFile)
        if not os.path.exists(obn + '_FA.nii.gz'):
            fit = fsl.DTIFit(base_name=obn, bvals=bvals, bvecs=bvecs, dwi=iFile, mask=nodif_brain, terminal_output='stream')
            fit.run()
            logFile = obn + '.log'
...
class knicrTBSS():
    ...
    def tbss_2_reg(self):
        ...
        if not os.path.exists(warp):
            fnirt = fsl.FNIRT(in_file=self.fa, fieldcoeff_file=warp, warped_file=oFile, affine_file=mat, config_file=config, ref_file=self.template, log_file=log)
            fnirt.run()
