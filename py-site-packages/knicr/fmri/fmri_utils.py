...
class gen_bootstrap_samples():
	...
	def gen_melodic_lisa(self, melodic_dir, melodic_sample_dir, sfix):
		nSamples = len(self.samples)
		for s in range(0, nSamples):
			oFile = os.path.join(melodic_sample_dir, 'sample.' + str(s) + '.' + sfix)
			if not os.path.exists(oFile):
				f = open(oFile, 'w')
				for l in self.samples[s]:
					f.write(os.path.join(melodic_dir, str(l), 'rs-160_idc_' + str(l) + '.ica', 'filtered_func_data.nii.gz' + '\n'))
				f.close()
			else:
