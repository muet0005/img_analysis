...
class knicrDTIprep():
    ...
    def resample_dti(self, voxel_size):
        oFile = self.dwi.replace('.nii.gz', '_' + str(voxel_size[0]) + 'mm.nii.gz')
        if os.path.exists(oFile):
            self.dwi = oFile
            return
        mri_convert = freesurfer.MRIConvert(in_file=self.dwi, out_file=oFile, vox_size=voxel_size)
        mri_convert.run()
    ...
    def fit_restore(self):
        oDir = os.path.join(self.dtifitDir, 'camino_restore')
        logFile = os.path.join(oDir, 'camino_restore.log')
        if not os.path.exists(oDir):
            os.makedirs(oDir)
        ero_mask = self.nodif_brain_mask.replace('.nii.gz', '_ero.nii.gz')
        fslmaths = fsl.maths.ErodeImage(in_file=self.nodif_brain_mask, out_file=ero_mask,  output_type='NIFTI_GZ')
        fslmaths.run()
        write_log(fslmaths.cmdline, logFile)
        ...
        if not os.path.exists(dwi_camino):
            ...
            img2vox = camino.Image2Voxel(in_file=self.data, out_file=dwi_camino, out_type='float', terminal_output='stream', ignore_exception=True)
            img2vox.run()
            write_log(img2vox.cmdline, logFile)
        mask_camino = os.path.join(oDir, 'mask.Bfloat')
        if not os.path.exists(mask_camino):
            print 'Converting mask to camino format: ', mask_camino
            img2vox = camino.Image2Voxel(in_file= self.nodif_brain_mask, out_file=mask_camino, out_type='float', terminal_output='stream', ignore_exception=True)
            img2vox.run()
            write_log(img2vox.cmdline, logFile)
        snr = os.path.join(oDir, 'estimatesnr.out')
        if not os.path.exists(snr):
            print 'computing sigma for RESTORE:'
            opts = ['estimatesnr', '-inputfile', dwi_camino, '-bgmask', mask_camino, '-schemefile', bvec_camino]
            stout = open(snr, 'w')
            p = sp.Popen(opts, stdout=stout)
            p.wait()
            stout.close()
        ...
        if not os.path.exists(restore_tensor):
            ...
            modelFit = camino.ModelFit(in_file=dwi_camino, model='restore', scheme_file=bvec_camino, bgmask=self.nodif_brain_mask, sigma=sigma, out_file=restore_tensor, ignore_exception=True)
            modelFit.run()
        ...
        for scalar in scalars:
            ...
            for tract in structList.keys():
                ...
                for stat in stats:
                    ...
                    if not stat == 'vol':
                        col = tract + '_dti_' + scalarType + '_' + stat + '_' + scalar 
                    else:
                        col_vol = tract + '_dti_vol'
                        col_vox = tract + '_dti_vox'
                    #print col, ':', val 
                    try:
                        #see if that column exists...if not...make it.
                        if col:
