...
class knicrDTIprep():
    ...
    def fit_restore(self, **kwargs):
        ...
        if not os.path.exists(restoreTensorNii):
            dt2nifti = camino.DT2NIfTI(in_file = restore_tensor, header_file=oFile, output_root=restoreTensorNii.strip('.nii.gz'), ignore_exception=True, terminal_output='stream')
            dt2nifti.run()
        #cleanup
