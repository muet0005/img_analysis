    ...
    def tbss4d(self, tbss_dir, oDir, template, **kwargs):
        ...
        for scalar in scalars:
            ...
            if scalar.endswith('_FA'):
                ...
                write_log(fslmaths.cmdline, logFile)
                op_str = '-Tmean'
                fslmaths = fsl.ImageMaths(in_file=merged4d, op_string=op_str, out_file=os.path.join(oDir, 'mean_FA.nii.gz'), output_type='NIFTI_GZ')
                fslmaths.run()
                write_log(fslmaths.cmdline, logFile)
                ...
                shutil.copyfile(merged4d, os.path.join(oDir, 'all_FA.nii.gz'))
            else:
