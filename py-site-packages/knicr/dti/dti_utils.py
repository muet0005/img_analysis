...
def compute_stat(iData, mask, **kwargs):
    ...
    if rescale:
        if np.max(mask_data) > 0:
            mask_data_below_thresh = mask_data < threshold
            mask_data[mask_data_below_thresh] = 0
    ...
    if stat == 'wavg':
        try:
            mask_data_below_thresh = mask_data < threshold
            mask_data[mask_data_below_thresh] = 0
            val = np.average(iData, weights=mask_data)
            ...
            for tract in structList.keys():
                ...
                for stat in stats:
                    ...
                    if stat == 'wavg':
                        ...
                        val = compute_stat(data, normPath, rescale=True, stat=stat)
                    else:
